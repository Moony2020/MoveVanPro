...
export default function Header({ currentView, setCurrentView, currentUser, onOpenLogin, onLogout }) {
  ...
  return (
    <header className="bg-white/95 backdrop-blur-md text-slate-900 border-b border-slate-200 sticky top-0 z-50 shadow-xs w-full max-w-full overflow-x-hidden transition-all">
      <div className="w-full max-w-[1440px] mx-auto px-3 sm:px-4 md:px-6 h-14 md:h-16 flex justify-between items-center gap-1.5 lg:gap-2">
        ...
        >
          <div className="w-8.5 h-8.5 md:w-9 md:h-9 rounded-lg bg-[#0058be] text-white flex items-center justify-center shadow-sm shadow-[#0058be]/20 border border-white/20 group-hover:scale-105 transition-all shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-1.5 xl:gap-2 shrink-0">
          ...
          {currentUser ? (
            ...
            <button 
              ...
              title="Log In"
              aria-label="Log In"
              className="flex items-center justify-center text-slate-800 bg-slate-100 hover:bg-slate-200/80 p-2 rounded-xl border border-slate-200 transition-all shrink-0 cursor-pointer"
            >
