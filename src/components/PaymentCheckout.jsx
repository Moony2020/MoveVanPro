...
export default function PaymentCheckout({ 
  ...
  return (
    <div className="bg-white border border-[#c2c6d6] rounded-3xl p-6 md:p-8 shadow-lg">
      
      {/* Header & Secure Environment Badge */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-6 border-b border-[#c2c6d6]/60 mb-6">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-black text-[#0b1c30]">Select Payment Gateway</h3>
          ...
          <p className="text-xs text-[#424754]">Choose your preferred checkout provider. All transactions are end-to-end encrypted.</p>
        </div>
      ...
      {showEnvKeyDetails && (
        <div className="mb-6 p-4 bg-[#0b1c30] text-white rounded-2xl border border-blue-900 text-xs space-y-2 animate-in fade-in duration-200">
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <IconComp className={`w-5 h-5 ${tab.color}`} />
              ...
              <div>
                <span className="text-xs font-black text-[#0b1c30] block">{tab.title}</span>
                <span className="text-[10px] text-[#424754] block">{tab.subtitle}</span>
              </div>
            </button>
          );
        })}
      ...
      {paymentMethod === 'stripe' && (
        <div className="space-y-4 bg-[#f8f9ff] p-5 rounded-2xl border border-[#c2c6d6]/80 animate-in fade-in duration-200">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="text-[10px] bg-blue-100 hover:bg-blue-200 text-blue-800 px-2 py-1 rounded-lg font-bold transition-colors cursor-pointer"
              >
                + Fill Visa Test Card
              </button>
              <button 
                ...
                className="text-[10px] bg-amber-100 hover:bg-amber-200 text-amber-800 px-2 py-1 rounded-lg font-bold transition-colors cursor-pointer"
              >
                + Fill Mastercard
              </button>
            </div>
          ...
          <div>
            ...
            <input 
              ...
              className={`w-full p-3 text-xs bg-white border rounded-xl focus:ring-2 focus:ring-[#0058be] focus:outline-none ${
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className={`w-full p-3 text-xs font-mono bg-white border rounded-xl focus:ring-2 focus:ring-[#0058be] focus:outline-none ${
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-[11px] font-bold text-[#424754] block mb-1">Expires (MM/YY)</label>
              <input 
                type="text" 
                value={cardExpiry}
                onChange={handleExpiryChange}
                placeholder="MM/YY" 
                className={`w-full p-3 text-xs bg-white border rounded-xl text-center focus:ring-2 focus:ring-[#0058be] ${
            ...
            <div>
              ...
              <input 
                ...
                placeholder="123" 
                className={`w-full p-3 text-xs font-mono bg-white border rounded-xl text-center focus:ring-2 focus:ring-[#0058be] ${
            ...
            <div>
              <label className="text-[11px] font-bold text-[#424754] block mb-1">Postcode / Zip</label>
              <input 
                type="text" 
                value={cardZip}
                onChange={(e) => setCardZip(e.target.value.toUpperCase())}
                placeholder="W8 7RG" 
                className="w-full p-3 text-xs bg-white border border-[#c2c6d6] rounded-xl text-center focus:ring-2 focus:ring-[#0058be]"
              />
            </div>
          </div>
        </div>
      ...
      {paymentMethod === 'paypal' && (
        <div className="space-y-4 bg-[#fffdfa] p-5 rounded-2xl border border-amber-300 animate-in fade-in duration-200">
          ...
          <div className="p-4 bg-white rounded-xl border border-amber-200 space-y-3">
            ...
            <input 
              ...
              className="w-full p-3 text-xs border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 font-mono"
            />
      ...
      {paymentMethod === 'applepay' && (
        <div className="space-y-4 bg-gray-900 text-white p-5 rounded-2xl border border-gray-800 animate-in fade-in duration-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
      ...
      {paymentMethod === 'driver' && (
        <div className="space-y-4 bg-emerald-50 p-5 rounded-2xl border border-emerald-200 text-emerald-900 animate-in fade-in duration-200">
          <div className="flex items-center gap-2">
      ...
      {/* Action CTA Button */}
      <div className="mt-6 pt-4 border-t border-[#c2c6d6]/60">
        <button
          onClick={handleExecutePayment}
          disabled={isProcessing}
          className="w-full bg-[#0058be] hover:bg-[#2170e4] text-white py-4 rounded-2xl font-extrabold text-sm flex items-center justify-center gap-2 cursor-pointer shadow-xl transition-all active:scale-[0.99] disabled:opacity-75"
        >
          {isProcessing ? (
            <>
            ...
            <>
              ...
              <span>Authorize & Lock Booking (${totalAmount}.00)</span>
              <ArrowRight className="w-4 h-4 ml-1" />
            </>
          )}
        </button>

        <div className="flex items-center justify-between text-[11px] text-[#424754] mt-3 px-1">
